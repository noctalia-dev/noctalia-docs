...
:::danger[Avoid QT_SCALE_FACTOR and QT_AUTO_SCREEN_SCALE_FACTOR]
Do **not** use `QT_SCALE_FACTOR` or `QT_AUTO_SCREEN_SCALE_FACTOR` to scale Noctalia. They cause numerous issues including misplaced tooltips, incorrect blur regions, and broken contextual menu positioning. Always use your compositor's per-monitor scaling instead.
:::
...
4. **Necessary environment variables not visible to Noctalia at start-up**: This affects people who do not set the contained environment variables:
   - Run:
   ```sh
