...
:::note[If you set these in Niri's configuration]
...
For example:
1. Disable the systemd service (if currently enabled):
  ```bash
  systemctl --user disable noctalia.service
  ```
2. Add a startup command to your `config.kdl`:
  ```ini
