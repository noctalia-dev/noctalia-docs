...
Systemd startup setup is now legacy-only and documented here: [Systemd Startup](/deprecated/systemd-startup/).
</details>
:::
...
```bash
QT_SCALE_FACTOR=2 qs -c noctalia-shell --no-duplicate
```
...
4. **Necessary environment variables not visible to Noctalia at start-up**: This affects Niri Systemd/UWSM/people who do not set the contained environment variables:
   ...
   - If you do not see any of these three you must make them available to Noctalia by adding them to a place where they can be seen in time:
      ...
      - in-config of your compositor for Niri without Systemd/UWSM-less method outlined in [Systemd Startup](/deprecated/systemd-startup/)
...
<details>
<summary><strong>Why is my battery not showing?</strong></summary>
...
<details>
<summary><strong>Audio Sample Rate Not Switching</strong></summary>
...
This is likely due to CAVA(Dependencie for the Visualizers) being installed as it has a tendencey to grab the first sample rate you it see and stay there.
