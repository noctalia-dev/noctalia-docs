...
Noctalia is packaged for [Arch Linux](#arch), [NixOS](/getting-started/nixos/), [Gentoo](#gentoo), [Fedora](#fedora), [Debian/Ubuntu](#debian), and [Void Linux](#void), but can also be [installed manually](#manual-install) in other distributions.
...
Noctalia provides an APT repository for Debian-based distributions (Debian Trixie or newer, Ubuntu 24.10+). This requires a system with Qt6 >= 6.6.
...
```bash
echo "deb [signed-by=/etc/apt/keyrings/noctalia.gpg] https://pkg.noctalia.dev/apt stable main" | sudo tee /etc/apt/sources.list.d/noctalia.list
```
...
Packages in this repository are signed. You need to trust the public key before installing:
...
**Step 3**: Sync and install noctalia-shell
