...
For more information on the available settings, see [the configuration reference](#config-ref).

:::note
If you provide a path or a JSON string, you **must** set all configuration options, since this overrides the *full* settings file without merging with defaults. Its good to backup your current config - found in `~/.config/settings.json` before trying. You can also use this file as a starting point for your new configuration. We hope to enable partial configuration with default fallbacks in the future.
:::
