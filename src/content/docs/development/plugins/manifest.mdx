...
:::tip
When providing a launcher provider, set `commandPrefix` in `metadata` to define the `>prefix` command. This is also used by `pluginApi.toggleLauncher()` for IPC integration. See [IPC Guide](/development/plugins/ipc/#launcher-provider-ipc) for details.
:::
