...
Desktop widgets support user-controlled scaling from 0.5x to 5x. **You must implement dimension-based scaling** rather than using Qt's transform-based `scale` property. Transform-based scaling causes blurry text and poor rendering at non-integer scale values.
