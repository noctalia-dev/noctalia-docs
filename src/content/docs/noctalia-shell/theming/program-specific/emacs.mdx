...
- **Theme is not found:** Ensure that Emacs theming is enabled in Noctalia settings and that the theme files are being generated correctly. If the files exist but aren't found, add `(add-to-list 'custom-theme-load-path "~/.config/emacs/themes/")` before `(load-theme 'noctalia t)`.
