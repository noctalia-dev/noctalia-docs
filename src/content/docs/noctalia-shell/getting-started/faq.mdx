...
Noctalia v4 is packaged for NixOS, Gentoo, Fedora, Debian (including Ubuntu and PikaOS), openSUSE, and Void Linux. The `noctalia-shell` and `noctalia-shell-git` packages are no longer in the AUR, so Arch users need to build and install v4 manually. See the [Installation guide](/noctalia-shell/getting-started/installation/) for details.

For other distributions, you can install manually but will need to build **[noctalia-qs](https://github.com/noctalia-dev/noctalia-qs)** (our custom fork of Quickshell) from source - the upstream `quickshell` package from your distro's repositories will not work. See the [manual installation instructions](/noctalia-shell/getting-started/installation/#manual-install) for steps.
...
Systemd startup setup is now legacy-only and documented here: [Systemd Startup](/noctalia-shell/deprecated/systemd-startup/).
</details>
:::
...
4. **Necessary environment variables not visible to Noctalia at start-up**: This affects Niri Systemd/UWSM/people who do not set the contained environment variables:
  ...
  - If you do not see any of these three you must make them available to Noctalia by adding them to a place where they can be seen in time:
     ...
     - in-config of your compositor for Niri without Systemd/UWSM-less method outlined in [Systemd Startup](/noctalia-shell/deprecated/systemd-startup/)
