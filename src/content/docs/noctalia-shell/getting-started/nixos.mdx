...
:::caution
To make Noctalia's Wi-Fi, bluetooth, power-profile, and battery feature available, please ensure the following NixOS options are enabled:
...
You can configure plugins declaratively. If an enabled plugin isn’t installed in the plugin directory, Noctalia will automatically install it when it starts up.
...
Keybindings are configured in your compositor. You make IPC calls to Noctalia [as explained in the keybinds section](/noctalia-shell/getting-started/keybinds/keybinds/). The only difference is that you can call `noctalia-shell` instead of `qs -c noctalia-shell`.
...
The old documentation is preserved for existing users [here](/noctalia-shell/deprecated/systemd-startup/).
...
There is no comprehensive documentation of all configuration options, but the default values are autogenerated and placed in a [`settings-default.json`](https://github.com/noctalia-dev/noctalia/blob/main/Assets/settings-default.json) in the Noctalia repository. We retrieve a live Nixified version of these here for your convenience:
