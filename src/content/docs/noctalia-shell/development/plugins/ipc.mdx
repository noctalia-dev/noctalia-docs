...
- **Launcher**: `pluginApi.openLauncher(screen)` / `pluginApi.closeLauncher(screen)` / `pluginApi.toggleLauncher(screen)` - Control the launcher for provider plugins
...
// Toggle launcher for provider plugins
pluginApi.withCurrentScreen(screen => {
...
:::tip
The `withCurrentScreen` helper is the preferred way to get a screen reference in IPC handlers. It automatically selects the appropriate screen based on the cursor position.
:::
...
## Launcher Provider IPC

If your plugin provides a launcher provider, you can use the launcher API methods to toggle the launcher from IPC. This works correctly in both normal and overlay launcher modes.
...
The `toggleLauncher` method uses the `commandPrefix` from your manifest's `metadata` to determine the search prefix. For example, with `"commandPrefix": "kaomoji"`, calling `toggle` will open the launcher with `>kaomoji ` pre-filled.

```bash
# Toggle the launcher with your plugin's command prefix
qs -c noctalia-shell ipc call plugin:my-provider toggle
```

:::tip
For plugins that have both a panel and a launcher provider, you can provide separate IPC methods for each:
...
IpcHandler {
  ...
  // Toggle the launcher with the provider's command prefix
  function toggle() {
    pluginApi.withCurrentScreen(screen => {
      pluginApi.toggleLauncher(screen);
    });
...
- [Main Component](/noctalia-shell/development/plugins/overview/#main-component) - Overview of Main.qml
- [Plugin API Reference](/noctalia-shell/development/plugins/api/) - Complete API documentation
