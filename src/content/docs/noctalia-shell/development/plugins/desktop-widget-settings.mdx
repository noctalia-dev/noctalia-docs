...
Unlike the general [Settings UI](/noctalia-shell/development/plugins/settings-ui/) which configures plugin-wide settings.
...
### `pluginApi`
The standard [Plugin API](/noctalia-shell/development/plugins/api/) object. Use it to access plugin-wide settings, translations, manifest data, and more.
...
As an alternative to calling `widgetSettings.save()`, your component can emit a `settingsChanged` signal. The settings dialog listens to this signal (debounced at 150ms) and handles persistence automatically. You can also emit `settingsSaved` for immediate (non-debounced) persistence:
...
## Best Practices
1. **Use for per-instance options only**: Desktop widget settings should control the appearance and behavior of a specific widget instance (background, colors, layout). Plugin-wide settings (API keys, global toggles) belong in the general `settings` entry point.
...
- [Bar Widget Development](/noctalia-shell/development/plugins/bar-widget/) - Create bar widgets
- [Desktop Widget Development](/noctalia-shell/development/plugins/desktop-widget/) - Create desktop widgets
- [Panel Development](/noctalia-shell/development/plugins/panel/) - Create overlay panels
