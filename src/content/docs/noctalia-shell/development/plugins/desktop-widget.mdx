...
For a complete guide, see [Desktop Widget Settings](/noctalia-shell/development/plugins/desktop-widget-settings/).
...
The `DraggableDesktopWidget` provides a default styled background with shadow. You can control it this way, but it's preferred to keep as `true` if you don't add your own custom background, so users can disable/enable it via the settings:
...
Desktop widgets support user-controlled scaling from 0.5x to 5x. **You must implement dimension-based scaling** rather than using Qt's transform-based `scale` property. Transform-based scaling usually causes jagged edges and overall poor rendering quality.
...
3. Add a new widget from the widget picker
4. Enable edit mode
