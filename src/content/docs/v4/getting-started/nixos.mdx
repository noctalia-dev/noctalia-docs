...
Keybindings are configured in your compositor. You make IPC calls to Noctalia [as explained in the keybinds section](/v4/getting-started/keybinds/keybinds/). The only difference is that you can call `noctalia-shell` instead of `qs -c noctalia-shell`.
...
The old documentation is preserved for existing users [here](/v4/deprecated/systemd-startup).
