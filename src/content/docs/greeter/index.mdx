...
The module enables greetd by default, enables `accounts-daemon` by default (for user avatars), and sets `default_session.command` to the package's `noctalia-greeter-session`. See [Configuration](/greeter/configuration/) for the full `greeter.toml` key list.
...
Example for a manual installation to `/usr/local` (replace the path if `which` shows something else, e.g. `/usr/bin/noctalia-greeter-session`):
...
`[session].default` in [greeter.toml](/greeter/configuration/)). The value is the session's
...
back to the last-used or the first-discovered session.
...
To skip the user list and open the password step for a specific account (similar to tuigreet's default user), set `[user].default` in [greeter.toml](/greeter/configuration/) or pass `--user` on the command line:
...
- a multi-monitor layout into `[output].layout` when more than one ready output reports distinct positions
...
**seatd without logind:** polkit has no graphical session to attach to, so Noctalia's polkit agent cannot register (`No session for pid` in the log). Sync Now stages files and shows the installation command. Run it from a terminal:

```sh
pkexec noctalia-greeter-apply-appearance "$XDG_RUNTIME_DIR/noctalia-greeter-sync"
```

To run Sync Now from the desktop on seatd, set a custom privilege prefix (the applied helper path and staging directory are appended automatically):
...
Synced data is installed under `/var/lib/noctalia-greeter/`: wallpaper image files, and a merge into `sync.toml` from a staged `sync.toml` fragment (palette, wallpaper refs, font, corner radius, session actions, scheme, layout/transforms/scales). Declarative `greeter.toml` is never overwritten by Sync; values set there win over Sync (including `[appearance.palette]` for the Synced scheme). See [Configuration](/greeter/configuration/).

After syncing, log out or restart greetd to see the changes on the login screen.

The greeter adds a **Synced** color scheme when `greeter.toml` or `sync.toml` has a complete palette. Session and scheme choices you make on the login screen are remembered in `/var/lib/noctalia-greeter/sync.toml` (unless the scheme is pinned in `greeter.toml`).

For monitor selection, output mode, idle blanking, UI scale, cursor theme, and other settings, see [Configuration](/greeter/configuration/).
...
- **Blank flash or modeset at login** - The greeter may be using a different DRM mode than your desktop session. Set matching `[output].width` and `[output].height` in `greeter.toml`. See [Output mode](/greeter/configuration/#output-mode).
- **Screen never blanks on the greeter** - Set `[idle].timeout` (seconds) in `greeter.toml`. Pointer motion alone does not count as activity. See [Idle blanking](/greeter/configuration/#idle-blanking).
