...
[Sync with Noctalia](/greeter/sync/#authorization) for status commands, NixOS examples,
...
See [Configuration](/greeter/configuration/) for all settings and [Sync with Noctalia](/greeter/sync/) for passwordless sync details.
...
For selecting the initial desktop session or user, see [Configuration](/greeter/configuration/).
