...
- [Build from source](/greeter/building-from-source/) if no package is available.
...
See [Troubleshooting](/greeter/troubleshooting/) for recovery steps and logging
details.
...
[Building from source](/greeter/building-from-source/). After installing, return to
...
- Install and enable **accountsservice** (usually the `accounts-daemon`
  ...
  no `IconFile` is set.
- Adjust the greeter in [Configuration](/greeter/configuration/), including the
  initial desktop session or user.
- Set up optional appearance sharing in
  [Sync with Noctalia](/greeter/sync/). That guide is the source of truth for
  compatibility, authorization, and security details.
- See [Displays](/greeter/displays/) and [Input and keyboard](/greeter/input/) for
