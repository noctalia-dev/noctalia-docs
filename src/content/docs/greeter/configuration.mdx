...
The **Synced** scheme uses a complete `[appearance.palette]` from `greeter.toml` when present, otherwise the same keys from `sync.toml`. Legacy live `appearance.json` is migrated into `sync.toml` once. See [Sync with Noctalia](/greeter/sync/) for the complete precedence and authorization model.
...
- [Displays](/greeter/displays/): connectors, layout, mode, transforms, scale, and idle blanking
- [Keyboard and cursor](/greeter/input/): navigation, XKB, Num Lock, and cursor themes
...
fingerprint. See [Fingerprint blocks password login](/greeter/troubleshooting/#fingerprint-blocks-password-login)
...
(`xterm`) otherwise. See
[X11 session doesn't start](/greeter/troubleshooting/#x11-session-doesnt-start) if
it still fails.
