...
| showFocusedScreen | false     | boolean | Display workspaces from the focused screen rather than the same screen as the bar. Useful for multi-head setup with a single status bar. |
