...
You can also turn the edit mode on from **Settings → Desktop → Toggle Editor**. Noctalia shows a short internal notification, and you should switch to an **empty workspace** (no windows covering the desktop) so the editor overlay is visible.
...
Clickable button with an optional glyph and label. Runs a shell command on the left click. Size auto-fits the content when `box_width` and `box_height` are `0.0`; when a box is set, the button stretches to fill it and scales its text and glyph to match.
...
Default-sink or default-source volume tile with a large glyph that fills bottom-up by volume percent. The left side shows the percentage, a Volume/Mic/Muted status line, and optionally the device name. Click the glyph to toggle mute; scroll anywhere on the widget to change volume (respects `[audio].enable_overdrive`). Requires PipeWire.
