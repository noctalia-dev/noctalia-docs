...
- **stretch** - scaled to the output's exact dimensions, ignoring the aspect ratio.
...
The launcher's wallpaper provider and **automatic rotation** use the same browse-folder resolution so they stay aligned with the picker. When automation runs with `per_monitor_directories = true`, each monitor picks independently of its own resolved folder.
