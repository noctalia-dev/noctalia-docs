...
- Internal Noctalia notifications (shell and plugins) are toast-only and are not stored in notification history, except [calendar reminders](/noctalia/services/calendar/#reminders).
...
- `history_retention_hours` removes stored notifications older than the configured number of hours. The default
...
Notifications can be filtered before toasts and sounds. External Freedesktop notifications can also be kept out of control-center history. Internal shell/plugin toasts use the same toast/sound filters, but they are not written to history (the save-history flag does not apply to them), except calendar reminders, which honor `save_history` like external notifications.
...
- `[notification.filter.<name>]` - per-sender rules evaluated in `filter_order` (first matching enabled filter wins). Each filter has:
 ...
 - `save_history` - keep the notification in control-center history (default `true`). Applies to external notifications and calendar reminders; other internal notifications are never stored.
