...
Community template discovery and updates require network access unless the selected template is already cached. [`shell.offline_mode`](/noctalia/shell/#shell-settings) prevents outgoing HTTP requests, including community template requests.
...
Relative paths are resolved from the Noctalia config directory. A leading `$XDG_CONFIG_HOME`, `$XDG_DATA_HOME`, `$XDG_STATE_HOME`, or `$XDG_CACHE_HOME` token is expanded using the XDG base directory defaults when the matching environment variable is not set.
