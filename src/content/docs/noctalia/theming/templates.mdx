...
`index` controls processing order and hook execution mode, and defaults to `0`. When `index = 0` (default), hooks execute asynchronously via a thread pool. Templates render without waiting for their hooks to finish.

When `index > 0`, hooks execute synchronously. Templates with higher index values are processed later, in ascending order. Use this when hook execution order matters or when a hook must complete before subsequent templates are processed.
