...
Because `settings.toml` loads last, it wins when it contains the same setting as your handwritten config layer.
...
:::tip
If you are confused about why a handwritten value is not taking effect, inspect or temporarily move `settings.toml` from the resolved state directory.
:::
...
The Settings action menu includes **Export Config...**. It opens a chooser with two export modes:
...
```
WARN  /home/you/.config/noctalia/bar.toml:14:10: bar.main.height: unknown setting
...
- [Shell](/noctalia/shell/) covers global behavior like fonts, scale, shadows, OSD, and keybinds.
- [Hooks](/noctalia/automation/hooks/) covers advanced event-driven automation.
- [Bars](/noctalia/bar/) and [Widgets](/noctalia/bar/widgets/) cover the top or sidebar and everything inside it.
