...
Right-click opens the Control Center unless you bind `right` to something else. While an attached panel is open, right-click anywhere on the bar still toggles Control Center unless the click lands in the dead zone and `right` is bound.
...
- On compositors that report overview state (Niri, Triad), the bar is kept visible while the overview is open.
