...
Hovering a named workspace shows its full name in a tooltip whenever the pill does not already spell it out: when `max_label_chars` cut it short, when `label_source = "id"` shows a number instead, or when labels are off. Set `show_tooltip = false` to turn this off.
...
| `show_tooltip` | bool | `true` | Show a named workspace's full name in a hover tooltip when its pill does not already display it. |
...
show_tooltip = true     # false = no full-name tooltip on hover
