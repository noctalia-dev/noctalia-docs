...
| `label_source` | string | `"id"` | What labels show: `"id"` (workspace number) or `"name"` (workspace name). |
| `max_label_chars` | int | `1` | Maximum characters for workspace name labels; longer names are truncated. Only applies when `label_source = "name"`, and only to non-numeric labels (workspace IDs like `10`, `11` are never truncated). |
