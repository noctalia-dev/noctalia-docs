...
The bar widget’s fullscreen action matches `pick` on multi-monitor setups. To capture a specific display from a script without a UI, pass the connector name (for example `noctalia msg screenshot-fullscreen DP-1`).
