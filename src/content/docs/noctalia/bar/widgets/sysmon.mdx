...
| `custom_label` | string | `""` | Custom text displayed beside the stat glyph or custom image. It remains visible when `show_glyph` is `false` and can be the widget's only visible content |
| `glyph_position` | string | `"before"` | Place the glyph or custom image and custom label group before or after the value or visualization |
...
`custom_label` is independent of the glyph slot. Keep `show_glyph` enabled to display the glyph or custom image beside
the label, or disable it for a text-only identity.
