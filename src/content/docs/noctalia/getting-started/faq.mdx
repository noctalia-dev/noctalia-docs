...
If you are using the Quickshell-based version, use the [v4 documentation](/noctalia-shell/) instead.
...
To disable that blur, keep the Noctalia `layer-rule` in your Niri config and set `blur false` inside its `background-effect` block. See [Niri compositor settings: Blur](/noctalia/compositor-settings/niri/?section=blur) for the full rule and notes.
