...
| `noctalia.offlineMode()` | bool | Returns `[shell].offline_mode`; plugins must not start network traffic while it is `true`. |
...
| `noctalia.wallpaperPath(connector)` | string or `nil` | Returns the exact wallpaper source configured for an output, or `nil` for an unknown output. |
| `noctalia.setWallpaperMask(connector, { path, wallpaperPath })` | nothing | Applies a source-aligned grayscale occlusion mask to every desktop-widget surface on the output. Requires plugin API 25. |
| `noctalia.setWallpaperMask(connector, nil)` | nothing | Clears this plugin runtime's mask from the output. Requires plugin API 25. |
