...
cache results are keyed by app id instead of resolving on every `update()`.
...
follows Noctalia's `NOCTALIA_STATE_HOME` override, and is created for you on the first call.
...
declaration. A `true` return means the request was accepted, not that the file decoded; a decoded failure calls the
