...
| `layer` | _(none, manifest only)_ | `top` (default) or `overlay`; only applies when floating (requires <PluginApiBadge feature="panel-layer" />) |
...
**Layer** is the plugin's call, not the user's: `shell.panel.floating_layer` covers the built-in panels only. `top`
keeps the panel under fullscreen windows, `overlay` keeps it above them, which is what an on-screen keyboard or a
heads-up panel needs. An attached panel ignores `layer` and sits at its host bar's configured layer.
