...
every desktop-widget script); each takes a prop table and an optional children array:
...
- **Markdown**: `ui.markdown` renders its `text` prop (headings, lists, code blocks, tables) as a read-only block.
  The host reparses only when `text` or the surface scale changes, so re-rendering a streaming tree with unchanged
...
- **Follow-scroll**: `ui.scroll` with `stickToBottom = true` stays pinned to the bottom while content grows, until
  ...
  the next layout passes, so it reaches content added in the same render. All three require
...
- **Cross-axis is the bar thickness**: the widget capsule clips to it, so keep the tree one control tall (row on a
  horizontal bar, column on a sidebar - branch on `barWidget.isVertical()`). Only the main axis grows with content.
