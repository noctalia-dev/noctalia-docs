...
Umbriel keeps VRR disabled if the output does not support adaptive sync or
rejects the request. Nested Wayland outputs normally depend on the parent
compositor and may not expose adaptive sync support.
