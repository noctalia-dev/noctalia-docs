---
title: Window Rules
description: Match application windows and configure their layout, appearance, focus, and fullscreen behavior.
---
...
[`appearance.opaque_fullscreen`](/umbriel/appearance/#window-appearance) is `false`.

### Border colors

Each `border_color_*` key is independent: an unset key keeps the
[`[colors.border]`](/umbriel/appearance/#border-colors) color. Combined with the state
...
`border_width`, `corner_radius`, and `shadow` change what Umbriel draws around
the windows a rule matches. Layout spacing keeps using the global border
widths, so a rule never moves other windows: a thinner border leaves its gap
empty, and a thicker one draws into the gap. `outer_border_width` and the
shadow's softness, offsets, and color stay global.
