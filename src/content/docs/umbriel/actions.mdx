---
title: Actions
description: Complete reference for keybind and IPC actions, including window, layout, output, overview, and scratchpad actions.
---

Actions are the compositor's verbs. Bind one under `[keybinds]`, attach it to a
[hot corner](/umbriel/keybinds/#hot-corners), or run it with `umbriel msg <action>`.
`umbriel msg --help` prints this same list, with the same wording and the same
grouping. See [Keybinds](/umbriel/keybinds/) for chord syntax and [IPC](/umbriel/ipc/) for the
...
Sizing rules per layout live in [Sizing behavior](/umbriel/layout/#sizing-behavior).
...
[Scratchpads](/umbriel/scratchpads/) covers their configuration, restoration rules, and
...
described in [Workspace selectors](/umbriel/workspaces/#workspace-selectors).
...
are described in [Overview](/umbriel/workspaces-overview/).
...
See [Layout](/umbriel/layout/) for geometry, directions, and resizing behavior.
...
- Floating and pinned behavior is described in [Layout](/umbriel/layout/) and
  [Scratchpads](/umbriel/scratchpads/).
