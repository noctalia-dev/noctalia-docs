---
title: Keybinds
description: Bind keys and mouse buttons to Umbriel actions - modifiers, parameterized actions, submaps, and Noctalia integration examples.
---

Configure bindings under `[keybinds]` and use this reference to understand their
syntax and behavior. See [Actions](/umbriel/actions/) for the complete action
reference.
...
Set a chord to `false` to disable an inherited binding. This is useful in the
final `settings.toml` overlay and works in any config source:
...
The chord is validated normally. Literal `true` is not a binding and is
reported as invalid.
...
layout list (see [Input](/umbriel/input/)), or bind the active layout's XKB keysym
name.
