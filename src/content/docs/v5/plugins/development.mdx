...
- `glyph` (a Tabler/Nerd-Font name) or `icon` (a themed icon name) is the row's leading visual. `badge` is a short
  ...
  secondary text.
- `setQuery(query)` replaces the open launcher's raw input text. A result-level `query` does the same when selected and
  keeps the launcher open, which is useful for autocomplete. Include your prefix if you want to stay routed to the same
  provider, e.g. `"/tr french "`.
