...
- Each result is a table: `{ id, title, subtitle?, glyph?, icon?, badge?, score? }`. `id` is passed back to
  `onActivate`. Results are ordered by `score` (descending), then insertion order.
