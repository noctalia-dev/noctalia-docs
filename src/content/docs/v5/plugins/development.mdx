...
| Subprocess | `runAsync(cmd [, cb])`, `runInTerminal(cmd)`, `commandExists(name)`, `processMatches(cb, ...needles)`, `flatpakAppInstalled(id)`, `portalAvailable()`, `getenv(name)`, `expandPath(path)` |
...
Filesystem and HTTP run safely off the UI thread; results arrive via callbacks.
