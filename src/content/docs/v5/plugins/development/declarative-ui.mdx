...
An explicit `fontSize`/`size`/`fontWeight` prop overrides the default. In bar widgets, `ui.button` renders compact —
it hugs its content instead of using the taller settings-panel control height (pass `width`/`height` to size it
explicitly).
