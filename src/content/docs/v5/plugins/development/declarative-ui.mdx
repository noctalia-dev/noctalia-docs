...
- **Hover**: `onHover = "name"` fires on pointer enter and leave for `ui.row`, `ui.column`, `ui.box`, `ui.image`, and
  `ui.button`. The global receives two strings: the state (`"true"` on enter, `"false"` on leave) and the node's `key`
  (empty when the node has none), so a single handler can serve a whole keyed list:
  ```lua
...
| `dismiss_on_outside_click` | _(none, manifest only)_ | Close when clicking outside (default `true`; requires `plugin_api = 8`) |
