...
- **Callbacks**: a callback prop takes either a **function** (requires `plugin_api = 9`) or the **name of a global
...
| `keyboard_focus` | _(none, manifest only)_ | `on_demand` (default), `exclusive`, or `none` (requires `plugin_api = 10`) |
| `persistent` | _(none, manifest only)_ | Keep open when another panel opens (requires `plugin_api = 11`) |
