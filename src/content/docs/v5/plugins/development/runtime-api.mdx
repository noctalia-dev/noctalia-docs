...
| `noctalia.isValidTimezone(name)` | bool | `true` when `name` is empty (system local) or names a zone in the active timezone database. Requires <PluginApiBadge feature="format-time-timezone" />. |
