...
`allow_insecure_tls` requires plugin API 7. When true, it disables certificate-chain and hostname verification for
...
`httpStream()` (plugin API 4) keeps the connection open and delivers the response body incrementally - use it for
