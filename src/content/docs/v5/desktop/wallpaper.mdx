...
Starred wallpapers appear at the **top of the browse grid** (before folders and images from the current directory). Star a tile to bookmark it; star again to remove it. The monitor selector (**ALL** vs one output) works the same for favorites and browse entries.
