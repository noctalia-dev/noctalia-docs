...
When enabled, Noctalia renders each desktop widget as its own tightly-sized layer-shell surface on the `Bottom` layer. The current implementation ships `clock`, `label`, `audio_visualizer`, `fancy_audio_visualizer`, `sticker`, `weather`, `media_player`, and `sysmon` widget types plus an interactive edit mode.
...
The fancy audio visualizer is a square, shader-based circular desktop widget with bars, wave, ring, and combined modes. The common `background` setting defaults to `false`; set `background = true` if you want the usual rounded desktop-widget background.
...
| `sensitivity` | float | `1.5` | Audio response sensitivity |
| `rotation_speed` | float | `0.5` | Rotation speed for animated circular elements |
| `bar_width` | float | `0.6` | Width of radial bars when bars are enabled |
| `ring_opacity` | float | `0.8` | Opacity of ring elements when rings are enabled |
| `bloom_intensity` | float | `0.5` | Glow intensity around visual elements |
| `wave_thickness` | float | `1.0` | Wave fill and edge thickness when waves are enabled |
| `inner_diameter` | float | `0.7` | Diameter of the center opening, from `0.0` to `1.0` |
| `fade_when_idle` | bool | `false` | Fade the visualizer out when no audio is playing |
