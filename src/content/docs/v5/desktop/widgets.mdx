...
The calendar widget uses the configured [calendar service](/v5/services/calendar/) accounts. It shows event indicators
...
The stats a `sysmon` widget can show, what each one displays, and where the values come from are listed under [Available stats](/v5/services/system-monitor/#available-stats). Desktop widgets support CPU frequency and every other stat except `ram_used` and the four disk stats.
