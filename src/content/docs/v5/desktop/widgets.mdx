...
For desktop widget edit-mode commands, see
[Shell & UI → Desktop widgets](/v5/ipc/shell-and-ui/#desktop-widgets).
...
| Drag a corner handle | Resize the tile (content scales to fill) |
...
A widget's tile size (`box_width` × `box_height`) is the grid's unit of layout: content is scaled to fill the tile,
so changing the tile size resizes the widget. When the snap grid is active, moving a widget snaps the nearest outer
edge or center line to the grid, screen edges, and nearby widgets, and resizing quantizes the tile to whole `cell_size`
cells on both axes — so widgets line up on the grid. With snapping off (grid hidden or `Shift` held) moving and
resizing are free/continuous. Resizing keeps the opposite corner anchored (hold `Alt` to resize about the center).
Toggling the grid never changes a widget's size.
...
| `box_width`, `box_height` | float | `0` | Tile size in logical pixels. `0` auto-fits the content's natural size; the editor's resize handle sets explicit values. Content is scaled to fill the tile (aspect-preserved) and centered. |
| `rotation` | float | `0.0` | Rotation in radians |

The old `scale` field (schema v1) is migrated automatically: a widget still using `scale` keeps its
size, and the next time you open the editor that size is baked into `box_width`/`box_height` and
`scale` is dropped.
...
| `font_family` | string | `""` | Font family for all text in the widget. Empty (or omitted) inherits the shell font (`[shell] font_family`). The editor offers a dropdown of installed families; the widget's bold/regular hierarchy is preserved with any family. A family that isn't installed on the current machine is still kept in the config. |
...
Desktop audio visualizers store an `aspect_ratio` setting for the bars' natural shape; the tile size (`box_width` × `box_height`) controls overall size. They also accept optional `mirrored`, `centered`, `show_when_idle`, `low_color`, and `high_color` settings; both colors default to `primary`.
...
Draws the current weather glyph alongside the temperature and short condition, driven by the shared `WeatherService`. Requires the resolved config to enable weather (`[weather] enabled = true`; via Settings or a `~/.config/noctalia/*.toml` file, see [Services — Weather](/v5/services/weather/)) — the widget will render a placeholder otherwise.
