...
| `type` | string | (required) | Widget type (`clock`, `weather`, `media_player`, etc.), or a plugin `[[desktop_widget]]` entry id (`author/plugin:entry`, e.g. `noctalia/timer:timer`) |
...
Plugins can ship their own desktop widget types via a `[[desktop_widget]]` manifest entry. Enabled plugin entries
appear in the edit-mode type picker alongside the built-in types, and are configured the same way — `type` is the
entry id and the `settings` table holds the plugin's declared settings:

```toml
[desktop_widgets.widget.kitchen_timer]
type = "noctalia/timer:timer"

[desktop_widgets.widget.kitchen_timer.settings]
duration = 600
```

See [Plugin Development](/v5/plugins/development/#desktopwidget--declarative-desktop-widget-ui) for authoring one.
...
| `format` | string | `"{:%H:%M}"` | Digital clock format string. Ignored when `clock_style = "analog"`. Supports the same syntax and tokens as the bar clock `format`; see [Date format tokens](/v5/configuration/date-format-tokens/). |
...
Desktop audio visualizers store an `aspect_ratio` setting for shape while `scale` controls overall size. They also accept optional `mirrored`, `centered`, `show_when_idle`, `color_1`, and `color_2` settings; both colors default to `primary`.
