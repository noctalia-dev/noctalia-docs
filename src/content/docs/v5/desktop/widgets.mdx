...
Desktop audio visualizers store an `aspect_ratio` setting for shape. `box_width` and `box_height` control the fitted widget box size; leave both `0.0` to auto-fit the content. They also accept optional `mirrored`, `centered`, `show_when_idle`, `color_1`, and `color_2` settings; both colors default to `primary`.
