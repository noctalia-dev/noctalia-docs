...
The stats a `sysmon` widget can show, what each one displays, and where the values come from are listed under [Available stats](/v5/services/system-monitor/#available-stats). Desktop widgets support every stat except `ram_used` and `disk_pct`.
