...
When `shell.panel.launcher_app_grid` is enabled and every visible result is an application, results switch from the default list to a multi-column icon grid with the app name underneath. Mixed searches (calculator hits, emoji, session actions, prefixed providers, and so on) keep the list layout. Left/right keybinds move between columns in grid mode; up/down move by row.
...
- **[Shell](/v5/shell/#shell-settings)**: `ui_scale` and `font_family` affect the launcher's appearance. `shell.panel.launcher_categories` controls the category filters. `launcher_show_icons`, `launcher_compact`, `launcher_app_grid`, and `launcher_sort_by_usage` adjust result layout and ordering (Settings → Panels → Launcher).
