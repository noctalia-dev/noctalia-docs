...
- **[Shell](/v5/shell/#shell-settings)**: `ui_scale` and `font_family` affect the launcher's appearance. `shell.panel.launcher_categories` controls the category filters. `launcher_show_icons` and `launcher_compact` adjust result rows (Settings → Panels → Launcher).
