...
- **[Shell](/v5/shell/#shell-settings)**: `ui_scale` and `font_family` affect the launcher's appearance. `shell.panel.launcher_categories` controls the category filters. `launcher_show_icons`, `launcher_compact`, and `launcher_sort_by_usage` adjust result rows and ordering (Settings → Panels → Launcher).
