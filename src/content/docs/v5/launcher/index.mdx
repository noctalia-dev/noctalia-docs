...
- **Terminal apps**: Entries with `Terminal=true` run inside a terminal. Prefer `export TERMINAL=wezterm` (or another emulator on `PATH`); do not put `-e` in `$TERMINAL`. See [Shell: launching apps](/v5/configuration/shell/#shell-settings) for discovery order and how this differs from `launch_apps_custom_command`.
...
(see [Shell settings](/v5/configuration/shell/#shell-settings)).
