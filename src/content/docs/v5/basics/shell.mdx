...
[osd.kinds]
volume = true            # output and input volume
...
Each popup kind can be toggled independently under `[osd.kinds]`. `lock_keys = false` disables Caps/Num/Scroll Lock popups; when no `lock_keys` bar widget is configured, this also stops
