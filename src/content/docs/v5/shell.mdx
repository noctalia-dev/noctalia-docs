...
- `shell.panel.open_near_click_control_center`, `open_near_click_launcher`, `open_near_click_clipboard`, and `open_near_click_wallpaper` position attached panels near the click location on the bar instead of centering them. Only effective when the corresponding `attach_*` toggle is also enabled.
