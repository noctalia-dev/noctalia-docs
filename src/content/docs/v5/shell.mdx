...
disable_mipmaps       = false           # startup-only; skip GPU mipmap generation
...
- `disable_mipmaps`: when `true`, skips GPU mipmap generation for all textures. Set this if icons and wallpapers show vertical stripe corruption on older GPUs (e.g. Intel HD 3000 / Sandy Bridge with Mesa crocus). Textures will use bilinear filtering only, which may look slightly less sharp when scaled down. Restart required.
