...
- `clipboard_image_action_command`: when non-empty, image clipboard previews show an action button. Use `{path}` to receive an exported image file path. Use `{stdin}` (expanded to `-`) or omit `{path}` to receive the image bytes on stdin, matching the v4 annotation tool behavior (`satty -f -`, `gradia`, etc.). After a command launches successfully, the clipboard panel closes.
...
- `shell.panel.launcher_placement`, `clipboard_placement`, `control_center_placement`, `wallpaper_placement`, and `session_placement` accept `attached`, `floating`, or `centered`. `attached` merges the panel with the host bar. `floating` keeps the panel detached but near the bar. `centered` opens it in the middle of the output. Defaults are centered for Launcher and Clipboard, attached for Control Center, Wallpaper, and Session.
...
for a taller vertical gauge. `lock_keys = false` disables Caps/Num/Scroll Lock popups; when no `lock_keys` bar widget
is configured, this also stops lock-key state polling.
...
Configure under **`[shell.session]`** in TOML, or use **Settings → Panels → Session menu**: entries, reorder, and **Show** are listed inline; use the **settings** (cog) control on a row for behavior, label, command, icon, and button style. **Add action** appends a custom-command entry by default.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `action` | string | *required* | `lock`, `logout`, `suspend`, `reboot`, `shutdown`, or `command` (custom entry). |
...
| `variant` | string | `default` | Button style: `default`, `primary`, `secondary`, `destructive`, `outline`, or `ghost`. The built-in default shutdown row uses `destructive`. |
