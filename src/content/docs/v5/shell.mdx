...
attach_session        = false  # attach session power menu to the bar when a suitable bar is available
...
open_near_click_session        = false  # open Session panel near the click on the bar
...
- `clipboard_enabled`: when `false`, disables Wayland clipboard integration (history, clipboard panel, bar/control-center shortcuts, and `Input` copy/paste hooks). Launcher math/emoji copy-to-clipboard actions are inactive while disabled.
...
- `shell.panel.transparency_mode` controls panel glass styling. `solid` keeps cards opaque and detached panels solid, `soft` makes detached panels lightly translucent and cards subtly translucent, and `glass` makes both visibly translucent while clamping card opacity high enough for readable text. Attached panels continue to use the effective panel background opacity inherited from the host bar.
- `shell.panel.attach_launcher`, `attach_clipboard`, `attach_control_center`, `attach_wallpaper`, and `attach_session` control whether those panels attach to the bar when a suitable bar is available. Attachment still requires the host bar to have `attach_panels = true`.
- `shell.panel.open_near_click_control_center`, `open_near_click_launcher`, `open_near_click_clipboard`, `open_near_click_wallpaper`, and `open_near_click_session` position attached panels near the click location on the bar instead of centering them. Only effective when the corresponding `attach_*` toggle is also enabled.
