...
launcher_browse_categories     = false  # optional segmented tabs for apps, wallpaper, and emoji browse
...
- `shell.shadow` defines the shared shadow metrics for shell surfaces and xdg-popup chrome such as panels, menus, pickers, and select dropdowns. Components such as panels, bars, and the dock only opt in/out with `shadow = true|false`; they do not define their own blur or offset.
...
- `shell.panel.shadow` toggles the rendered drop shadow on attached, floating, and centered panels. Shadow blur, offset, and alpha still come from `[shell.shadow]`.
- `shell.panel.launcher_placement`, `clipboard_placement`, `control_center_placement`, `wallpaper_placement`, and `session_placement` accept `attached`, `floating`, or `centered`. `attached` merges the panel with the host bar when the bar allows `attach_panels`; `floating` keeps the panel detached but near the bar; `centered` opens it in the middle of the output. Defaults are centered for Launcher and Clipboard, attached for Control Center, Wallpaper, and Session.
- When multiple enabled bars resolve to the same edge on one output, exact source-bar attachment is ambiguous. In that case attached/floating panels use the compositor's reserved edge area and open outside the bar stack instead of trying to merge with one bar.
...
- `shell.panel.launcher_browse_categories` adds optional segmented tabs in the launcher for browsing providers that opt in (applications, wallpaper, emoji). The **All** tab still merges global providers (applications and calculator). Prefixed queries such as `/emo` or `/wall` still route to the matching provider and override the selected tab.
...
makes volume, brightness, and lock-key popups 25% larger than the default). `lock_keys = false` disables Caps/Num/Scroll
Lock popups; when no `lock_keys` bar widget is configured, this also stops lock-key state polling. `keyboard_layout = false`
