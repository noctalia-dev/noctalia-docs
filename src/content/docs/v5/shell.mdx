...
The OSD powers the volume HUD and defaults to `top_right`. `lock_keys = false` disables Caps/Num/Scroll Lock popups;
when no `lock_keys` bar widget is configured, this also stops lock-key state polling.
