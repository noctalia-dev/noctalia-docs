...
- `time_format` and `date_format` are fallbacks for shell-owned displays such as the dashboard, calendar tab, and lock screen. Widget-specific clock formats still use their own widget settings. See [Date format tokens](/v5/configuration/date-format-tokens/).
