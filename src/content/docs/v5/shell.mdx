...
When `wlr-screencopy` is available, Noctalia can capture the desktop **before** the session lock engages and show that frame blurred behind the lock UI instead of the wallpaper.

```toml
[lockscreen]
blurred_desktop = false   # capture desktop via screencopy at lock time
blur_intensity  = 0.5     # 0.0 – 1.0
tint_intensity  = 0.2     # surface-color overlay on top of the blur
```

If capture fails (missing protocol, permission denied, etc.), the lock screen falls back to the normal per-output wallpaper. The snapshot lives in memory only until unlock.
