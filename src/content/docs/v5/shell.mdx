...
- `shell.shadow` defines the shared shadow metrics for shell surfaces and xdg-popup chrome such as menus, pickers, and select dropdowns. Components such as bars and the dock only opt in/out with `shadow = true|false`; they do not define their own blur or offset.
