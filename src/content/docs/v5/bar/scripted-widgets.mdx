...
A `bongocat.lua` script is bundled in `assets/scripts/` (with its `bongocat.otf` font in `assets/fonts/`). A cat sits in your bar and slaps its paws when you type, naps when idle, and blinks.
