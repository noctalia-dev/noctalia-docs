...
| `scope` | string | `"instance"` | Runtime scope. `"instance"` creates one script runtime per bar widget instance. `"shared"` creates one runtime per widget config key and shares its displayed state across all matching bars. |
