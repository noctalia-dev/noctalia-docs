...
A **negative** per-corner radius carves a concave corner instead of a convex
...
In **Settings → Bar → Shape** you don't type a negative number: each per-corner slider
shows the positive radius, and an **invert** toggle beside it carries the sign, turning
it on stores the negative value that produces the concave spike. The toggle is enabled
only for the two inner corners (below); on the outer, screen-edge corners it appears
disabled, since a concave spike there has no room to grow.

Concave spikes only render on the bar's two **inner** corners, the corners facing away
from the screen edge, because the bar can't grow along the edge it's anchored to:

| Bar position | Inner corners (invertible) |
| --- | --- |
...
(screen-edge) corner has no room to grow and is clipped (in the config file), so leave
those at `0` or a positive radius.
...
| `interactive` | bool | `true` | Set `false` to make the widget passive: hover highlight and tooltips are disabled, and pointer events (clicks, scrolls) pass through to the bar underneath. The widget is still drawn — use `enabled = false` to hide it entirely. |
...
2. **Drag one widget onto the middle of another** (using its drag handle) to instantly create a group from the two.
   Or tick the checkbox on two or more **adjacent** widgets and click **Group**. Either way they move into one
   group container and a style editor opens.
...
| `drawer_enabled` | bool | `false` | Replace the group's inline members with a drawer button that opens them in a panel. |
| `drawer_detached` | bool | `false` | Open the drawer as a detached panel instead of keeping it attached to the bar. |
| `drawer_glyph_closed` | string | `chevron-down` | Drawer button glyph when the panel is closed. |
| `drawer_glyph_opened` | string | `chevron-up` | Drawer button glyph when the panel is open. |
| `drawer_columns` | number | `3` | Number of widget slots per row in the drawer panel (`1` to `5`). |
