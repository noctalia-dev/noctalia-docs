...
layer              = "top"       # top | overlay — attached panels use the same layer
...
Only the fields you specify are overridden; everything else falls through to the `[bar.*]` defaults. Supported override fields: all bar fields including `position`, plus `auto_hide`, `reserve_space`, `layer`, `scale`, `background_opacity`, `color`, and all `capsule_*` keys.
...
`layer` selects the wlr-layer-shell layer for the bar (`top` or `overlay`). Attached panels (for example Control Center when it attaches to the bar) are placed on the same layer as the bar that opened them. Centered or floating panel placements keep each panel's own layer (typically overlay).
