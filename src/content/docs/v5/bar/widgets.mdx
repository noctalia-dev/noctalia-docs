...
| `warning_threshold` | integer | `20` | Battery percentage at or below which glyph and label switch to warning color. `0` disables. |
...
| `scope` | string | `"instance"` | `"instance"` runs one runtime per bar widget instance. `"shared"` runs one runtime per widget config key and shares state across bars. |
