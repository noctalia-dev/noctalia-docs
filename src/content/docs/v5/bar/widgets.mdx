...
Middle-click a bar widget to open Settings directly to that widget's configuration. Disable `[shell].middle_click_opens_widget_settings` if you want middle-click to remain available to plugin widgets.
...
| `enable_scroll` | bool | `true` | Allow mouse wheel and trackpad scrolling to switch to the adjacent workspace. |
...
Left-click an icon to focus that window (or launch a pinned app that is not running). Middle-click an icon to close that window (the active window when a grouped or pinned multi-window tile represents several). Middle-click in the gaps between icons still opens the taskbar widget settings when `[shell].middle_click_opens_widget_settings` is enabled. The active window shows a small primary dot under the icon.
...
**Pinned apps** (`pinned`) keep desktop entries on the taskbar when they are not running. Matching uses the same desktop-entry rules as the [dock](/v5/dock/#pinned-matching). Pinned apps appear first (in list order), collapsed to one icon per app; other running windows follow as usual. Right-click an icon and choose **Pin** / **Unpin** to update the list. Non-running pins are dimmed by `pinned_opacity` (default `0.5`). Pins are only used on the flat strip — when `group_by_workspace` is on they are ignored (settings hide the list, and pin menu actions are unavailable).
...
With grouping, `workspace_group_content` chooses what each capsule shows for its windows: `icons` (default app tiles), `count` (window count), or `dots` (one dot per window; the focused window uses the primary color when `show_active_indicator` is on). Dot mode shows at most five dots and a `+N` overflow when there are more windows. Left-click on count/dots activates that workspace. `group_single_icon_per_app` only applies when content is `icons`.
...
| `enable_scroll` | bool | `true` | Allow mouse wheel and trackpad scrolling to switch to the adjacent window (or workspace when `group_by_workspace` is on). |
...
| `enable_scroll` | bool | `true` | Allow mouse wheel and trackpad scrolling to change volume |
...
| `enable_scroll` | bool | `true` | Allow mouse wheel and trackpad scrolling to skip to the previous/next track |
...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `enable_scroll` | bool | `true` | Allow mouse wheel and trackpad scrolling to change brightness |
| `scroll_step` | int | `5` | Percentage change applied on each mouse wheel step (1–25) |
...
| `enable_scroll` | bool | `true` | Allow mouse wheel and trackpad scrolling to run the scroll commands |
...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `enable_scroll` | bool | `true` | Allow mouse wheel and trackpad scrolling to cycle power profiles |
