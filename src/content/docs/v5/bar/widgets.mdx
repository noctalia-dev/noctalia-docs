...
Middle-click a bar widget to open Settings directly to that widget's configuration. Every click, scroll and thumb button on every widget can be rebound - see [Widget Actions](/v5/bar/actions/).
...
[`[shell.privacy]`](/v5/shell/#privacy-capture-filters).
...
Left-click cycles layouts using the compositor backend when supported. Bind `left` to an `exec` action to run a custom command instead - see [Widget Actions](/v5/bar/actions/).
...
Shows a custom glyph/image and an optional label. What it does on click and scroll is set with [widget actions](/v5/bar/actions/).
...
The capture **output policy** (save to file, output directory, filename pattern, clipboard, freeze, pipe command) is global and lives under [`[shell.screenshot]`](/v5/shell/#shell-settings), shared by this widget and the IPC commands. The widget only sets its glyph; the left click is a [widget action](/v5/bar/actions/).
