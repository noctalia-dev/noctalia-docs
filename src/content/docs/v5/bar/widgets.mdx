...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `display_mode` | string | `"glyph"` | `"glyph"` shows a Tabler glyph; `"graphic"` draws a rectangular battery shape with an animated fill level. |
| `show_label` | bool | `true` | In `glyph` mode: show the percentage label next to the glyph. In `graphic` mode: overlay the percentage text on the battery body. |
...
| `warning_color` | color | `"error"` | Color applied when charge is at or below the configured `[battery]` warning threshold. Ignored while charging. |
...
The system battery low-battery threshold is configured under `[battery]`. The same threshold controls when the default system battery widget uses `warning_color`. Peripheral batteries do not inherit this value; configure a per-device threshold for each peripheral that should warn. Do not add a per-device table for the default system battery; `[battery].warning_threshold` is its canonical threshold.

```toml
[battery]
warning_threshold = 20 # 0 disables low-battery warnings

[battery.device."/org/freedesktop/UPower/devices/gaming_input_hid_001"]
