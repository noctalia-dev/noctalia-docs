...
| `focused_color` | string | `"primary"` | Color role for the focused workspace pill; fixed hex colors are also supported |
| `occupied_color` | string | `"secondary"` | Color role for occupied (non-focused) workspace pills; fixed hex colors are also supported |
| `empty_color` | string | `"secondary"` | Color role for empty workspace pills; fixed hex colors are also supported |
| `hide_when_empty` | bool | `false` | Hide workspace pills with no open windows. |

Prefer color role names so widgets follow the active palette. Fixed hex colors (`#RGB`, `#RGBA`, `#RRGGBB`, `#RRGGBBAA`) are supported when a non-palette color is intentional. Invalid role names or malformed hex colors are treated as config errors.
...
| `low_color` | string | `"primary"` | Color role for the first bars; fixed hex colors are also supported |
| `high_color` | string | `"primary"` | Color role for the last bars; fixed hex colors are also supported |
...
| `show_label` | bool | `true` | In `icon` mode: show the percentage label next to the glyph. In `graphic` mode: show the battery value beside or below the battery graphic. Vertical bars omit the `%` suffix. |
...
In `graphic` mode, charging and plugged states show a small state glyph near the label, or inside the graphic when `show_label = false`.
...
# Graphical battery with percentage label
[widget.battery]
type = "battery"
...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `hide_label` | bool | `false` | Show only the keyboard glyph, hiding the layout text label |
| `display` | string | `"short"` | `"short"` = compact code; `"full"` = full layout name. Ignored when `hide_label = true`. |
...
Static command button for simple user actions. Use `scripted` when the widget needs command output, polling, dynamic
state, IPC callbacks, or custom logic.

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `glyph` | string | `"heart"` | Bar glyph name: Noctalia alias, Tabler icon name, or `U+` / `0x` codepoint. Empty hides the glyph. |
| `label` | string | `""` | Optional static label shown beside or below the glyph. |
...
| `middle_command` | string | `""` | Shell command run on middle click. When set, middle click runs this command instead of opening widget settings. |
| `scroll_up_command` | string | `""` | Shell command run when scrolling up over the button. |
| `scroll_down_command` | string | `""` | Shell command run when scrolling down over the button. |
...
glyph = "volume"
label = "Vol"
scroll_up_command = "wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%+"
scroll_down_command = "wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-"
```
