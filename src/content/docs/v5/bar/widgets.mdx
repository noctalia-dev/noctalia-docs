...
| `primary_click` | string | `"region"` | Left-click action: `"region"` (interactive drag selection) or `"fullscreen"` (all outputs) |
...
Region mode shows a dimmed overlay; drag to select, release to capture, **Escape** to cancel.
...
IPC uses the same output settings as the bar widget (`directory`, `filename_pattern`, `save_to_file`, `copy_to_clipboard`, `pipe_to_command`, `pipe_command`). With no widget name, the first screenshot widget on a bar is used (bar list order, then `[widget.*]` names). Pass a widget name to target a specific `[widget.<name>]` entry.
