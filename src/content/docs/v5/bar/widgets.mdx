...
Left-click an icon to focus that window. Middle-click an icon to close that window (the active window when a grouped app tile represents several). Middle-click in the gaps between icons still opens the taskbar widget settings when `[shell].middle_click_opens_widget_settings` is enabled. The active window shows a small primary dot under the icon.
