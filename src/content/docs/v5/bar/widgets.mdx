...
| `gauge_color` | string | `"primary"` | Color role (or fixed hex) for the gauge fill bar when `display = "gauge"` |
| `highlight_color` | string | `"error"` | Color role (or fixed hex) the value (icon, label, and graph line) is tinted toward as the stat rises through its [thresholds](/v5/services/system-monitor/#value-highlighting) |
