...
Please add this to your `hyprland.lua` for intended Alt Tab behaviour:
...
Please note that some of the follwing overlay keybinds might not work when the keybind is set to `ALT + TAB`:
- **Arrow keys** - move within the grid (up to five columns)
- **Enter** - focus the selected window and close
- **Escape** - close without switching
