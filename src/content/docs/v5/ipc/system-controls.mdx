...
| Action | Command | Description |
|--------|---------|-------------|
| Set power profile | `noctalia msg power-set balanced` | Activate the named profile. If the service reports a profile list, the name must match one of those entries exactly. |
| Cycle power profile | `noctalia msg power-cycle` | Switch to the next profile in UPower's ordered list (same order as the bar power widget and control center shortcut). Wraps from the last profile back to the first. Takes no arguments. |
