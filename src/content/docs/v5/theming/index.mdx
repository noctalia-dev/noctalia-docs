...
Palette changes apply live when the resolved config changes, whether through Settings or edits to `~/.config/noctalia/*.toml`. Wallpaper-derived themes re-resolve automatically when the default wallpaper changes. See [Palette](/v5/theming/palette/) for color roles, custom palette files, and terminal colors.
...
schedule in [`[location]`](/v5/services/location/) (same rules as [Night Light](/v5/services/night-light/)): sunrise and
sunset computed from your coordinates, or your own `sunset` / `sunrise` times when
[`custom_schedule = true`](/v5/services/location/#custom-schedule). Night light does not need to be enabled for theme
...
[Media & UI → Theme](/v5/ipc/media-and-ui/#theme).
...
See [Palette](/v5/theming/palette/) for the custom palette JSON format.
...
- [App Theming](/v5/theming/app-theming/) covers applying Noctalia colors to app config files.
- [Template Reference](/v5/theming/templates/) covers template syntax, tokens, filters, hooks, and CLI rendering.
- [FAQ: external wallpaper apps](/v5/getting-started/faq/#how-do-i-use-noctalia-colors-with-an-external-wallpaper-app) covers wallpaper-driven colors when another tool displays the wallpaper.
