...
schedule in [`[location]`](/v5/services/#location) (same rules as [Night Light](/v5/services/#night-light)): weather
coordinates when `use_weather_location = true`, otherwise manual `sunset` / `sunrise` times, otherwise computed times
from `latitude` / `longitude`. Night light does not need to be enabled for theme auto to follow the schedule. When the
schedule cannot be resolved, `auto` uses the light (day) palette until a valid schedule is available.
