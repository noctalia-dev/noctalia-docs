...
`community_ids` selects which community templates run. Metadata and files are stored in `~/.local/state/noctalia/community-templates/` (honoring `NOCTALIA_STATE_HOME`, then `XDG_STATE_HOME`).
...
Community template discovery and updates require network access unless the selected template is already cached. [`shell.offline_mode`](/v5/shell/#shell-settings) prevents outgoing HTTP requests, including community template requests.
...
User template entries support the same fields documented in [Template Entries](/v5/theming/templates/#template-entries), including `input_path_modes`, `output_path_dynamic`, `colors_to_compare`, `compare_to`, `pre_hook`, `post_hook`, and `index`.
...
For template syntax, color tokens, filters, hooks, dynamic output paths, and CLI rendering, see the [Template Reference](/v5/theming/templates/).
