...
A palette is the set of colors selected by [`[theme]`](/v5/theming/). The theme config chooses the source and mode; the palette provides the actual color values used by bars, panels, widgets, templates, and other theme-aware UI.
...
Settings role pickers expose these role names first. Template files access these roles and the full Material token set through the [Template Reference](/v5/theming/templates/#available-color-tokens).
