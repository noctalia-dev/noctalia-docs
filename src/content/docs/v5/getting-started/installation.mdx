...
Gentoo cannot currently keep Noctalia v4 and v5 installed at the same time. You still need to mask Noctalia v4 before emerging v5.
:::
...
**Step 3**: [Mask](https://wiki.gentoo.org/wiki/Knowledge_Base:Masking_a_package) Noctalia v4:
...
:::caution[Alpha builds]
The Debian repository currently ships alpha builds built directly from the `main` branch. There are no stable releases yet.
:::
...
:::note
The APT repository provides `amd64` packages only. The same binary is served across all supported distributions.
:::
