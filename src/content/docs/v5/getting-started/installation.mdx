...
The Noctalia team maintains only the **Arch Linux/AUR packages** and the **manual installation method**. Packages shipped in a distribution's official repositories, such as Fedora's official package, are built and reviewed under that distribution's own packaging process.
...
<Tabs>
  <TabItem label="Arch">
    ...
    sudo pacman -S meson gcc just \
      ...
      pam curl libwebp libjxl librsvg libqalculate
    ```
  </TabItem>
...
`libjxl` handles JPEG XL decoding, and Wuffs handles the other supported raster image formats.
