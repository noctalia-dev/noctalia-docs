...
To install Noctalia v5, first [add the input](#add-input) using your preferred input-pinning method. Then you can [install the package](#install-package) directly or use either the [Home Manager](#home-manager) or [Hjem](#hjem) modules.
...
The Home Manager module allows you to configure Noctalia's settings [(that you would otherwise put in `~/.config/noctalia/`)](/v5/configuration) using nix instead of toml, or allows you to link an existing toml file. It is optional.
