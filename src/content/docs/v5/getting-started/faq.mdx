...
If you manage wallpapers with another tool such as waypaper, swww, or hyprpaper, disable Noctalia's wallpaper rendering and keep wallpaper-driven theme generation enabled:
...
Weather requires `[weather] enabled = true` plus a resolved location — set `[location] auto_locate = true` or an `[location] address`. The weather widget also requires the weather service to be enabled.

Weather, Theme `auto`, and Night Light all share the `[location]` source (`auto_locate`/`address` coordinates, manual `latitude`/`longitude`, or fixed `sunset`/`sunrise`). Night Light also needs compositor support for the `wlr-gamma-control` protocol.
