...
:::note[KDE Plasma]
Add `noctalia` under System Settings → Autostart, or see [KDE Plasma](/v5/compositor-settings/kde/) for Plasma-specific limits (clipboard, blur, screenshots, and more).
:::
...
See the [IPC overview](/v5/ipc/) and [Shell commands](/v5/ipc/shell/).
...
Yes, when the lock screen is enabled. Noctalia holds a logind sleep delay inhibitor and locks the session on `PrepareForSleep` before allowing suspend or hibernate to proceed (including lid-close sleep). You do not need a separate idle daemon for lock-before-sleep.
