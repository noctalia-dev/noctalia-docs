...
Theme `auto` and Night Light share the `[location]` schedule (weather coordinates, manual `sunset`/`sunrise`, or `latitude`/`longitude`). Night Light also needs compositor support for the `wlr-gamma-control` protocol.
