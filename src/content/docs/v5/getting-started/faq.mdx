...
<FaqItem question="Will Noctalia v4 automatically upgrade to v5?">

No. Noctalia v5 is a fresh install, not an automatic upgrade from the Quickshell-based v4 line. It ships as a separate package, your package manager will not replace v4 with v5, and v4 settings are not migrated.

If you are moving from v4, expect to recreate your compositor keybinds for opening the launcher, panels, and other Noctalia commands. The good part is that v5 ships with sane defaults, so you can get started quickly and customize from there.

See the [Installation guide](/v5/getting-started/installation/) and [Running Noctalia](/v5/getting-started/running-the-shell/) for setup steps.
...
See [Widget definitions](/v5/bar/widgets/#widget-definitions) for more examples.
...
See [Shell settings](/v5/configuration/shell/#shell-settings) and [Bars](/v5/bar/).
...
To disable that blur, keep the Noctalia `layer-rule` in your Niri config and set `blur false` inside its `background-effect` block. See [Niri compositor settings: Blur](/v5/compositor-settings/niri/?section=blur) for the full rule and notes.
...
See [Bars](/v5/bar/) for the full per-bar and per-monitor field list.
...
See the [`battery` widget docs](/v5/bar/widgets/#battery).
...
See [Location](/v5/services/location/), [Theme](/v5/theming/#mode), and [Night Light](/v5/services/night-light/).
