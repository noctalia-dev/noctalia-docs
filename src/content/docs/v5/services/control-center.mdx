...
Set `compact = true` under `[control_center]` to show only icons in the sidebar, hiding the tab labels. This makes the Control Center panel narrower.
...
|--------|------|---------|-------------|
| `compact` | bool | `true` | Show only icons in the sidebar (no tab labels) |
...
The Clipboard panel shows history entries with persistent pin markers for pinned items. Destructive actions use a
two-click confirmation: click the trash action once to arm it, then click again to delete. The header trash action clears
only unpinned history entries, so pinned entries stay protected from bulk clearing. To delete a pinned item, select that
entry and use its individual delete action.
