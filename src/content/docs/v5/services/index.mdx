...
gpu_temp_poll_seconds = 2.0
gpu_vram_poll_seconds = 2.0
memory_poll_seconds = 1.0
swap_poll_seconds = 1.0
...
Poll intervals are configured globally under `[system.monitor]`, not per widget. Each value is the number of seconds between samples for that metric group. Values are clamped to `0.1`–`120` seconds. You can also adjust them in **Settings → Services** when the system monitor is enabled.

| Key | Default | Metrics |
|-----|---------|---------|
| `cpu_poll_seconds` | `1.0` | CPU usage %, CPU temperature, and load average (`1` / `5` / `15` min) |
| `gpu_temp_poll_seconds` | `2.0` | GPU temperature |
| `gpu_vram_poll_seconds` | `2.0` | GPU VRAM usage |
| `memory_poll_seconds` | `1.0` | RAM usage % and used/total MB |
| `swap_poll_seconds` | `1.0` | Swap usage % and used/total MB |
| `network_poll_seconds` | `1.0` | Network download and upload throughput |
| `disk_poll_seconds` | `5.0` | Disk usage % for paths used by bar `sysmon` widgets with `stat = "disk_pct"` |
| `history_poll_seconds` | `1.0` | Graph history cadence for bar and desktop `sysmon` widgets in graph mode |

CPU usage and CPU temperature share `cpu_poll_seconds` because both are tied to the same CPU sampling pass. Network Rx and Tx share `network_poll_seconds` because they come from the same `/proc/net/dev` read.
