...
offset_x           = 0      # extra horizontal offset from screen edge (px)
offset_y           = 0      # extra vertical offset from screen edge (px)
...
- `offset_x` / `offset_y` add extra distance from the screen edge, relative to the toast position. For example, with `position = "top_right"`, `offset_x` pushes toasts further from the right edge and `offset_y` pushes them further from the top edge.
