...
Poll intervals are configured globally under `[system.monitor]`, not per widget. Each value is the number of seconds between samples for that metric group. Values are clamped to `0.1`–`120` seconds. You can also adjust them in **Settings → Services → Advanced** when the system monitor is enabled.

| Key | Default | Metrics |
|-----|---------|---------|
| `cpu_poll_seconds` | `1.0` | CPU usage %, CPU temperature, and load average (`1` / `5` / `15` min) |
| `gpu_poll_seconds` | `2.0` | GPU temperature and VRAM usage |
| `memory_poll_seconds` | `1.0` | RAM usage % and used/total MB |
| `network_poll_seconds` | `1.0` | Network download and upload throughput |
| `disk_poll_seconds` | `5.0` | Disk usage % for paths used by bar `sysmon` widgets with `stat = "disk_pct"`, plus swap usage % and used/total MB |
...
Bar and desktop `sysmon` widgets in **graph** mode scroll using the same cadence as the fastest poll interval above (the minimum of the configured intervals). There is no separate graph-only setting.
