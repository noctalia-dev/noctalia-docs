---
title: Services
description: Audio, brightness, location schedule, night light, weather, calendar, idle behaviors, and notifications.
sidebar:
  order: 7
---
...
Shared sunrise/sunset schedule for [Theme `auto`](/v5/theming/#mode) and [Night Light](#night-light). Configure under **Settings → Services → Location**, directly below Weather.
...
# Option B: manual geolocation schedule (fallback when sunset/sunrise are missing)
# latitude  = 52.5200
# longitude = 13.4050
```

When `use_weather_location = true`, Noctalia uses WeatherService coordinates for sunrise/sunset once weather has a configured location (`[weather].auto_locate = true` or `[weather].address`). While weather coordinates are unavailable, and whenever `use_weather_location = false`, it uses `sunset` + `sunrise`, then falls back to explicit `latitude`/`longitude`. If only one coordinate is provided, the schedule is considered invalid.
