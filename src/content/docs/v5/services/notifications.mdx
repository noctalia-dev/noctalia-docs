...
The empty app match and `.*` content regex match every external notification. Because the first matching enabled filter
wins, keep `no_history` first in `filter_order` if you add other filters. In Settings, leave **App** empty, enter `.*`
for **Content Regex**, and disable **Save to history**.
