...
`[calendar]` covers accounts and sync only. How the Calendar tab is drawn, including event date and time formats and the ISO week number column, lives under [`[control_center.calendar]`](/v5/control-center/#calendar).
...
When upgrading from a version that stored calendar credentials in `state.toml`, Noctalia moves Secret Service-backed CalDAV passwords and Google refresh tokens into Secret Service once. A file-backed account's old plaintext password is discarded because its configured file is authoritative. Old access tokens are also discarded. The plaintext owner table is removed only after every credential that requires migration is stored successfully. If the provider is unavailable, locked, or the prompt is cancelled, Settings shows **Calendar Credential Storage** with a retry action. Calendar sync does not read pending plaintext values.

If Secret Service contains a matching credential but it remains locked after an unlock attempt, Settings reports the
credential store as locked instead of treating the item as missing. If a configured Google account has no refresh token,
its account row shows **Reconnect**. The encrypted event-cache master key is separate from account credentials, so a
cache-key warning does not by itself mean the Google authorization was lost.
...
[Encrypted storage master key](/v5/configuration/shell/#encrypted-storage-master-key). When no storage key is available,
live calendar sync continues but events are not read from or written to disk. A legacy plaintext `events.json` is
deleted only after its contents have been parsed and an authenticated encrypted replacement has been written
successfully. Missing or incorrect keys preserve existing files and surface a retry action in Settings.
...
If Settings reports that Secret Service is unavailable, confirm that a compatible provider is running in the same login session, then use the retry action. If access is locked or denied, unlock the provider and retry. A cancelled prompt is not treated as a missing credential; start the operation again when you are ready.
