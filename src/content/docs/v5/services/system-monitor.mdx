...
A stat whose source is unavailable on the machine - GPU VRAM on an integrated GPU, for example - is shown as unavailable rather than as zero. The GPU stats are also disabled by default: they report nothing until `gpu_poll_seconds` is set to a non-zero value, and what each GPU vendor can report is covered under [GPU data sources](#gpu-data-sources).
