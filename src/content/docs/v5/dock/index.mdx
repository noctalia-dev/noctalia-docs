...
Dock application icons follow the global `[shell.app_icon_colorize]` / `[shell.app_icon_color]` settings (see [Shell → App icon colorization](/v5/shell/#app-icon-colorization)). Bar launcher/control-center `custom_image` uses a separate alpha-mask path; glyph fallbacks are unchanged.
