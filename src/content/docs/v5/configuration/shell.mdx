...
session_search = false  # include session actions in general search, not only behind /session
...
prefix  = "/ssh"                   # route behind a prefix; leave empty for global only
...
| `grid` | bool | `false` | When `true`, displays the session panel actions in a multi-row grid based on `grid_columns`. |
| `grid_columns` | number | `3` | The number of actions to show per row when `grid` is `true`. |
