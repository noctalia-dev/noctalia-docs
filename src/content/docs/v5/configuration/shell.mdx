...
launcher_categories            = true   # show launcher category filters; F6 reveals, then cycles
...
- `shell.panel.launcher_categories` shows compact category filters in the launcher for providers that expose categories, currently Applications and Emoji. **All** leaves results unfiltered. With a prefixed provider such as `/emo`, the filters come from that provider. Press `F6` while the launcher is open to reveal hidden filters without changing the active filter. Once the filters are visible, press `F6` to move to the next filter or `Shift+F6` to move to the previous filter.
