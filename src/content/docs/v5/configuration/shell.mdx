...
popup_borders         = true            # draw outlines around popups and dropdowns
...
prefix = ""              # calculator has no default trigger, so it stays in global search
global = true            # set false to only show calculator results behind a custom prefix
...
- Clipboard history is encrypted at rest with a dedicated data key. The default `secret-service` source stores it through Secret Service. If the provider is unavailable or locked, history still works for the current session but is not read from or written to disk; unlock or start the provider and use **Settings → Shell → Clipboard → Retry**. Existing plaintext history is migrated only after the encrypted replacement is complete, and encrypted files are preserved if their key is missing.
...
- `popup_borders`: when `true` (default), popups and dropdowns (context menus, select dropdowns, dialogs, and tray/dock menus) draw a subtle outline that separates them from the surface they float over. Set it to `false` for borderless popups. Toggle in **Settings → Appearance → Borders**.
...
- When multiple enabled bars resolve to the same edge on one monitor, exact source-bar attachment is ambiguous. In that case attached/floating panels use the compositor's reserved edge area and open outside the bar stack instead of trying to merge with one bar. Prefer setting `shell.panel_anchor_bar` when you have more than one bar and open panels via IPC or shortcuts.
...
- `shell.launcher.provider_prefix` sets the common prefix character used to trigger all provider prefixes (default `"/"`). See [Launcher: Provider prefixes](/v5/launcher/#provider-prefixes) for how prefixes, trigger words, and global search fit together.
- `shell.launcher.providers.<name>.prefix` sets the trigger word for a specific built-in provider (`calculator`, `emoji`, `session`, `wallpaper`, `windows`; keys are matched case-insensitively). The provider is activated when typing the `provider_prefix` followed by this trigger word (e.g. `/session` or `/emo`). Setting the prefix to an empty string (`""`) falls back to the provider's built-in default trigger — it does **not** make the provider global. The `calculator` provider has no default trigger, so it stays in global search. The Applications provider cannot be prefixed and is always queryable globally.
...
`shell.clipboard_storage.key_source = "file"` makes the configured `key_file` authoritative. Noctalia does not look up,
create, edit, or delete a Secret Service key for clipboard storage in this mode. The path must be absolute and resolve
to a regular file containing exactly 64 lowercase hexadecimal characters (32 random bytes), with an optional final LF.
This works with agenix, sops-nix, and systemd credentials as long as the decrypted file is readable by the user running
Noctalia.
...
```sh
umask 077
head -c 32 /dev/urandom | xxd -p -c 64 > noctalia-clipboard-key
```

Keep this key stable. Replacing it does not rotate existing history: authentication fails, Settings reports a storage
error, and Noctalia preserves the encrypted files. Restore the original key to reopen them. If the configured file is
missing or invalid, clipboard history remains session-only; after provisioning the correct file, use
**Settings → Shell → Clipboard → Retry**.
...
`orientation = "vertical"` for a taller vertical gauge on volume and brightness sliders; text-only popups (media, lock
...
OSDs by default; set `volume_output` or `volume_input` to `false` to disable one side only. `lock_keys = false` disables Caps/Num/Scroll Lock popups; when no `lock_keys` bar widget is configured, this also stops
...
Requires [Noctalia Greeter](/v5/greeter/) and `noctalia-greeter-apply-appearance` on `PATH`. When those are missing, auto-sync stays off, Security settings for greeter sync are hidden, and the `greeter-sync` IPC command is not registered.
...
|-----|------|---------|-------------|
| `auto_sync` | `bool` | `false` | Automatically sync the greeter whenever wallpaper, colors, or theme mode change. Ignored when the greeter is not installed. |
...
See [Noctalia Greeter](/v5/greeter/) for seatd and polkit notes.
