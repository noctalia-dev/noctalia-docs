...
- **TOML syntax errors** — with the file, line, and column.
- **Unknown sections and settings** — misspelled or obsolete keys that would otherwise be ignored, including bar, widget, and desktop-widget settings (validated against each widget type's known options).
- **Bad values** — wrong type, invalid color, or a value outside the allowed range or enum set.

Errors mean the configuration is invalid and the command exits with status `1`; warnings (such as an out-of-range number that the shell would clamp) are advisory and do not fail the check. A clean config prints `✓ Config is valid` and exits `0`, which makes it convenient in a dotfiles pre-commit hook or CI step.
