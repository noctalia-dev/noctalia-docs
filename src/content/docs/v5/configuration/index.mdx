...
- **Noctalia's** - `~/.local/state/noctalia/` (GUI overrides, UI state, plugin repo caches, exported plugin files, and
  downloaded catalogs).
- **Credential provider's** - durable calendar credentials managed by Secret Service, or CalDAV password files managed
  by an external provisioner such as agenix. Deleting Noctalia's state directory does not remove them.
