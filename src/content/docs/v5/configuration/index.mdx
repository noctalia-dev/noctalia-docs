...
Files under `~/.config/noctalia/` are never rewritten. Values that need migration are normalized in memory on every
load, and Settings keeps a warning visible until you update the source TOML. Noctalia also sends a notification when
it first finds an affected path, when a new affected path appears, and every three days while affected values remain.

Run the validator for every affected path and its canonical replacement:
...
| Clipboard encryption key | Desktop Secret Service or an explicit key file | opens encrypted clipboard history |
...
- **Noctalia's** - `~/.local/state/noctalia/` (GUI overrides, UI state, plugin repo caches, exported plugin files, and
  downloaded catalogs).
- **Credential provider's** - durable calendar credentials and the clipboard encryption key managed by Secret Service,
