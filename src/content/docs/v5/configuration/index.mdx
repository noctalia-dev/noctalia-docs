...
| Plugin source clones | `~/.local/state/noctalia/plugins/sources/` | git-source clones, re-fetchable |
...
- **Noctalia's** — `~/.local/state/noctalia/` (everything the app writes or fetches: GUI overrides, UI state, plugin
  clones, downloaded catalogs) — app-managed and regenerable, so it's always safe to delete.
