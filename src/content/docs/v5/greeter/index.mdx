...
For monitor selection, output mode, idle blanking, UI scale, cursor theme, and other settings, see [Configuration](/v5/greeter/configuration/).
...
- **Wrong session on startup** - If `[session].default` is set in `greeter.toml`, it wins over last-used `[session].last` in `sync.toml`. Run `noctalia-greeter sessions` for exact **Name** spelling.
