...
- **Wrong or default cursor theme** - Set `cursor_theme` (and `cursor_size`) in `greeter.conf`, or the `XCURSOR_*` variables in the greetd command. If the theme is not on the default search path, also set `cursor_path` / `XCURSOR_PATH`. See [Cursor theme](/v5/greeter/configuration/#cursor-theme).
