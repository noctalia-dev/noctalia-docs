...
- **Blank screen** - Check greetd / journal logs (stderr by default). If using `NOCTALIA_GREETER_LOG`, check that file. Ensure `/var/lib/noctalia-greeter` exists (`just setup-log-dir` from the greeter source tree, or create it owned by the greetd user).
...
- **Black screen after reboot** - With default logging, inspect the greetd service journal (or your init's capture of session stdout/stderr).
