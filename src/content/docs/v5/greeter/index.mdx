...
To skip the user list and open the password step for a specific account (similar to tuigreet's default user), set `default_user` in [greeter.conf](/v5/greeter/configuration/) or pass `--user` on the command line:
...
With Noctalia v5 and the greeter both installed, open **Settings → Security → Noctalia Greeter → Sync Now**. Noctalia copies your wallpaper and palette to the greeter (you will be prompted for admin access via polkit, through `pkexec` or `run0`).
