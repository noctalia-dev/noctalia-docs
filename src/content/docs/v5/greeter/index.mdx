...
With Noctalia v5 and the greeter both installed, open **Settings → Security → Noctalia Greeter → Sync Now**. Noctalia copies your wallpaper, palette, shell font (`[shell] font_family`), enabled session actions (`shell.session` plus `[shell.session.power]` overrides), monitor orientation (`[output].transforms`), and (when you use multiple monitors) monitor layout to the greeter (you will be prompted for admin access via polkit, through `pkexec` or `run0`).
...
Enable **Settings → Security → Auto-Sync Greeter** to automatically sync whenever your wallpaper, colors, theme mode, or shell font change. Syncing triggers after a short debounce so rapid changes (like dragging a color picker) are batched into one sync.
