...
**Settings → Security → Noctalia Greeter → Sync Now** copies palette colors and wallpapers into `/var/lib/noctalia-greeter/`.

With a current Noctalia shell and greeter:

| File / field | Purpose |
|--------------|---------|
| `wallpaper.<ext>` + `appearance.json` → `wallpaper` | Legacy **single** image (always written) |
| `wallpaper-<connector>.*` + `appearance.json` → `wallpapers` | **Per-connector** map (`DP-2`, `HDMI-A-1`, …) |

The greeter picks the image for each view by the bound connector name. If you pin with `[output].name = "DP-2"`, you see the DP-2 wallpaper (when that entry exists).

Older greeters that only understand the single `wallpaper` field keep working: Sync still fills that fallback. When `[output].name` is set, the shell prefers that connector’s wallpaper for the fallback image as well.

No extra greeter.toml keys are required for wallpapers — only Sync Now (and optional pin as above).
