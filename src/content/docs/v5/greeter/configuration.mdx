...
Noctalia **Sync Now** also writes `[output].transforms` from your desktop compositor's reported output transforms (including `normal`). You can still set them by hand in `greeter.toml` (or via your Nix module). Restart greetd after changes:
