...
The `DraggableDesktopWidget` provides a default styled background with shadow. You can control it this way but it's prefered to keep as `true` if you don't add your own custom background so users have the ability to disable/enable it via the settings:
