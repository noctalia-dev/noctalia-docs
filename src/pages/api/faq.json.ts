...
export const GET: APIRoute = async () => {
  try {
    ...
    const faqPages = await getCollection('docs', ({ id }) => {
      return id === 'v5/getting-started/faq';
    });
    ...
    const filePath = path.join(projectRoot, 'src', 'content', 'docs', 'v5', 'getting-started', 'faq.mdx');
    ...
    try {
      ...
      try {
        const altPath = path.join(process.cwd(), 'src', 'content', 'docs', 'v5', 'getting-started', 'faq.mdx');
